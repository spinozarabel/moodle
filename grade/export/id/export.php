...
require_once 'grade_export_id.php';

$id                = required_param('id', PARAM_INT); // course id
$PAGE->set_url('/grade/export/id/export.php', array('id'=>$id));
...
require_capability('gradeexport/id:view', $context);
...
if (!empty($CFG->gradepublishing) && !empty($key)) {
    print_grade_page_head($COURSE->id, 'export', 'id', get_string('exportto', 'grades') . ' ' . get_string('pluginname', 'gradeexport_id'));
}
...
$data = $mform->get_data();
$export = new grade_export_id($course, $groupid, $data);
...
if (!empty($CFG->gradepublishing) && !empty($key)) {
    ...
    $event = \gradeexport_id\event\grade_exported::create(array('context' => $context));
    $event->trigger();
    $export->print_grades();
}
